...
function getRelevantGoods() {
	$.ajax({
		url: '//coco70.51wnl.com/numberologynew/BaseCeSuan/GetRelevantGoodsList?size=3&type=0',
		type: 'get',
		dataType: 'json',
...
$(document).on('click', '.jinshegnContentDetail,.bottomFixBtnFree', function () {
	...
	location.href = '//order.51wnl.com/pay_web/index_t.html?money=' + money + '&source=' + source + '&orderName=' + orderName + '&parterid=' + parterId + '&goodsid=' + goodsId + '&parteruserid=' + unionId +
		...
		success: function (res) {
			...
			if (!isPay) {
				...
				if (/payresult=1/.test(location.href)) {
					var shareUrl = '//mobile.51wnl.com/numberology/qsjs/index.html?userId=WNLUSERID&deviceId=OPENUDID&pushToken=PTOKEN&pToken=PTOKEN&mac=MAC&imei=IMEI&idfa=IDFA&channel=CHANNEL&posId=posId&boundId=BUNDLE';
					/* var shareUrl = "//mobile.51wnl.com/numberology/qsjs/index.html"; */
					shareRedPackage({
						goodsId: 'F4B189A3A2A447DB828F2D6A4FB40FDC', //正式
						/* goodsId: 'C96997A0DCDF436CB902ED40E0840AB2', //测试 */
