(function(global) {
	'use strict';
	console.log('init the wnlHistory');
	...
	global.couponId = getQueryValue('couponId') || localData.couponId || '';
	global.openid = '';
	global.unionid = '';

	posId = (/posid/i).test(posId) ? '' : posId;
	boundId = (/bundle/i).test(boundId) ? '' : boundId;
	pushToken = (/Token/i).test(pushToken) ? '' : pushToken;
	pToken = (/Token/i).test(pToken) ? '' : pToken;
	channel = (/channel/i).test(channel) ? '' : channel;
	mac = (/mac/i).test(mac) ? '' : mac;
	imei = (/imei/i).test(imei) ? '' : imei;
	idfa = (/idfa/i).test(idfa) ? '' : idfa;
	couponId = (/couponId/i).test(couponId) ? '' : couponId;
	...
	if(browser.isWx()) {
		...
		if (wnl_loc && wnl_loc.wnlUserId) {
			...
			if(localStorage.getItem('history_couponId')){
				couponId = localStorage.getItem('history_couponId');
			}
			var data = {
