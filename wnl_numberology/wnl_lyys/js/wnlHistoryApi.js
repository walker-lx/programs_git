...
(function(global) {
	'use strict';
	var historyBaseUrl = '//coco70.51wnl.com/numberologynew/cs/list.html';
	var ua = window.navigator.userAgent;
	var isWnl = ua.toLowerCase().indexOf('wnl') > -1;
	var localData = localStorage.getItem('wnl_numerology_historyPro_data');
	var browser = {
		isAndroid: function() {
	...
	global.posId = getQueryValue('posId') || getQueryValue('posid') || localData.posId;
	global.pToken = getQueryValue('pToken') || getQueryValue('ptoken');
	global.pushToken = getQueryValue('pushToken') || getQueryValue('pushtoken');
	global.mac = getQueryValue('mac');
	global.imei = getQueryValue('imei');
	global.idfa = getQueryValue('idfa');
	global.boundId = getQueryValue('boundid') || getQueryValue('boundId');
	global.couponId = getQueryValue('couponId') || localStorage.getItem('history_couponId') || '';
	console.log('#-----1---' + couponId)
	global.channel = getQueryValue('channel');
	global.openid = '';
	global.unionid = '';
	global.clientType = '';
	if (browser.isIOS()) {
		global.clientType = 'IOS';
	} else if (browser.isAndroid()) {
		global.clientType = 'Android';
	} else {
		global.clientType = 'other';
	}

	localStorage.setItem('history_couponId', couponId);
	console.log('#-----2---' + couponId)
	posId = (/posid/i).test(posId) ? '' : (posId=='null'?'':posId);
	boundId = (/bundle/i).test(boundId) ? '' : (boundId=='null'?'':boundId);
	pushToken = (/Token/i).test(pushToken) ? '' : (pushToken=='null'?'':pushToken);
	pToken = (/Token/i).test(pToken) ? '' : (pToken=='null'?'':pToken);
	channel = (/channel/i).test(channel) ? '' : (channel=='null'?'':channel);
	mac = (/mac/i).test(mac) ? '' : (mac=='null'?'':mac);
	imei = (/imei/i).test(imei) ? '' : (imei=='null'?'':imei);
	idfa = (/idfa/i).test(idfa) ? '' : (idfa=='null'?'':idfa);

	if (userId !== '') localData.userId = userId;
	if (deviceId !== '') localData.deviceId = deviceId;
	if (posId !== '' ) localData.posId = posId;
	if (pushToken !== '') localData.pushToken = pushToken;
	if (pToken !== '') localData.pToken = pToken;
	if (mac !== '') localData.mac = mac;
	if (imei !== '') localData.imei = imei;
	if (idfa !== '') localData.idfa = idfa;
	if (boundId !== '') localData.boundId = boundId;
	if (channel !== '') localData.channel = channel;
	if (clientType !== '') localData.channel = clientType;

	localStorage.setItem('wnl_numerology_historyPro_data', JSON.stringify(localData));

	var historyBtn = global.document.createElement('a');
	historyBtn.innerHTML = '我的订单';
	historyBtn.setAttribute('class', 'wnl_history_btn');
	var cssText1 = '.wnl_history_btn{font-size:16px;line-height:16px;position: fixed;box-sizing: border-box;text-align: right;z-index: 1000;right: 10px;width: 112px;bottom: 10px;height: 40px;border-radius: 20px;-webkit-backdrop-filter: blur(10px);backdrop-filter: blur(10px);background-color: rgba(0, 0, 0, 0.6);box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.1);color: #FFF;text-decoration: none;padding: 12px;padding-right: 15px;}';
	...
	if (!isWnl) {
		// historyBtn.style.bottom = '65px'; 因为没有下载banner了，可以让非客户端的按钮和客户端位置一致。
		...
		setTimeout(function() {
			// 去掉protocol请求，主要观察android客户端是否有异常
	...
	if (!browser.isWx() && (!browser.isWnl() || (userId === '' && deviceId === ''))) {
		if (localStorage.getItem('wnl_tlp_guid')) {
			window.userId = localStorage.getItem('wnl_tlp_guid');
			window.deviceId = localStorage.getItem('wnl_tlp_guid');
			historyBtn.style.display = 'block';
		}
		else {
			$.ajax({
				...
				success: function(response) {
					window.userId = response.toString();
					window.deviceId = response.toString();
	...
	// 微信客户端打开 1.如果有本地存储数据则直接读取 2.如果没有本地存储数据&没有openid则进行微信授权
	// 3.授权回调第三方登录获取用户数据并保存到本地
	if(browser.isWx() && location.hostname.indexOf('51wnl') !== -1) {
	...
	else if (browser.isWx() && location.hostname.indexOf('51wnl') === -1) {
		var userIdObj = JSON.parse(window.localStorage.getItem('wx_guid'));
		userId = userIdObj ? userIdObj.wnlUserId : btoa((Date.parse(new Date()) + Math.random()*1000).toString());
		deviceId = userId;
		window.localStorage.setItem('wx_guid', JSON.stringify({
			wnlUserId: userId,
			openid: '',
			unionid: ''
		}));
