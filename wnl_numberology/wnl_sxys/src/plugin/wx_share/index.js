...
class wxShare {
  ...
  constructor(params) {
    this.config = {
      ...
      weixinhandler: '//b.cqyouloft.com/interface/API/weixinhandler.ashx',
    ...
    this.loadScript(this.config.jweixin).then(e => {
      return window.promise.get(this.config.weixinhandler, {
        ...
        wx.config({
          ...
          jsApiList: this.config.jsApiList // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
        });
        wx.ready(() => {
          console.log('wechat share is ready!');
          this.setWxParams(params);
        });
        // console.log(wx)
        wx.error((e) => {
  ...
  setShareParams(params) {
    if (!window.wx) {
      console.error('wechat is not ready');
      return false;
    }
    wx.ready(() => {
      console.log('set wechat params by params');
      this.setWxParams(params);
    });
  }
}
