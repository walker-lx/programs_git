(function (global) {
  ...
  // 鍒ゆ柇闈炲井淇′笅 && userId 涓� deviceId 涓虹┖ || 涓嶆槸wnl瀹㈡埛绔墦寮€鏃� 璋冪敤guid鐢熸垚鎺ュ彛
  console.log('check');
  ...
  function addCSSText(cssText) {
    var style = document.createElement('style'), //鍒涘缓涓€涓猻tyle鍏冪礌
      head = document.head || document.getElementsByTagName('head')[0]; //鑾峰彇head鍏冪礌
    style.type = 'text/css'; //杩欓噷蹇呴』鏄剧ず璁剧疆style鍏冪礌鐨則ype灞炴€т负text/css锛屽惁鍒欏湪ie涓笉璧蜂綔鐢�
    if (style.styleSheet) { //IE
      var func = function () {
        try { //闃叉IE涓璼tylesheet鏁伴噺瓒呰繃闄愬埗鑰屽彂鐢熼敊璇�
          style.styleSheet.cssText = cssText;
        } catch (e) {
          console.log(e);
        }
      };
      //濡傛灉褰撳墠styleSheet杩樹笉鑳界敤锛屽垯鏀惧埌寮傛涓垯琛�
      ...
      //w3c娴忚鍣ㄤ腑鍙鍒涘缓鏂囨湰鑺傜偣鎻掑叆鍒皊tyle鍏冪礌涓氨琛屼簡
      var textNode = document.createTextNode(cssText);
      style.appendChild(textNode);
    }
    head.appendChild(style); //鎶婂垱寤虹殑style鍏冪礌鎻掑叆鍒癶ead涓�
  }
