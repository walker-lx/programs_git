...
Page({
  ...
  onLoad: function (options) {
    ...
    //orderId = '646f2d0146cb43649a45c9f40d523cf2'; //已支付
    console.log('resultOrderID' + orderId);
    console.log('clientType' + clientType);
    //获取订单详情
    wx.request({
      url: 'https://coco70.51wnl.com/NumberologyNew/NRLorder/GetOrderAnswer?orderid=646f2d0146cb43649a45c9f40d523cf2',
      ...
      success: res => {
        ...
        if (res.data.data.isPayed) {
          ...
          wx.request({
            ...
            method: 'GET',
            success: res => {
              let miniResultData = res.data.data;
              console.log('~~~~~~~' + miniResultData.name);
              headerInfo.push({
                'name': miniResultData.name,
                'sex': miniResultData.sex,
                'glYear': miniResultData.birth.split(' ')[0].split('-')[0],
                'glMonth': miniResultData.birth.split(' ')[0].split('-')[1],
                'glDay': miniResultData.birth.split(' ')[0].split('-')[2],
                'glHour': miniResultData.birth.split(' ')[1].substring(0, 2),
                'bzYear': miniResultData.baZi.split(' ')[0],
                'bzMonth': miniResultData.baZi.split(' ')[1],
                'bzDay': miniResultData.baZi.split(' ')[2],
                'bzHour': miniResultData.baZi.split(' ')[3],
                'xingQing': miniResultData.xingQing,
              });
              _this.setData({
                headerInfo: headerInfo,
              })
