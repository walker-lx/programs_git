...
(function(global) {
	...
	if (!browser.isWx() && (!browser.isWnl() || (userId === '' && deviceId === ''))) {
		if (localStorage.getItem('wnl_tlp_guid')) {
			...
			// 	localStorage.setItem('wnl_tlp_guid', window.userId);
	...
	if(browser.isWx()) {
		...
		if (wnl_loc && wnl_loc.wnlUserId) {
			...
			var xhr = new XMLHttpRequest();  
			xhr.open('POST', '//u.51wnl.com/Login/OpenLogin?cid=Youloft_Android&av=4.2.6&mac=00:11:22:33:44:55&idfa=b622c089e7e14d2c2fa8c9129dafbb51&did=b622c089e7e14d2c2fa8c9129dafbb51&chn=wnl_anzhi&cc=CN&lang=zh&bd=com.youloft.calendar');  
			xhr.onreadystatechange = function (response) {
				console.log('get the info');
					window.userId = response.data.wnlUserId;
					window.deviceId = response.data.wnlUserId;
