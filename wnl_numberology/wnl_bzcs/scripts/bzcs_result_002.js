$(function () {
	...
	$("#btnViewDetail").click(function () {
		if (share) {
			...
			location.href = 'http://order.51wnl.com/pay_web/index_t.html?money=98&source=八字测算&orderName=八字测算&parterid=NRLorder&goodsid=41DBA1789A644753A408CD78DAF79B00&parteruserid=' + uniqueId +
				'&data=' + orderId + '&posId=' + posId + '&openid=' + openid + '&couponId=' + couponId + '&imei=' + imei + '&returnUrl=' + encodeURIComponent('http://' + location.host + location.pathname + '?orderid=' + orderId);
					...
					success: function (result) {
						if (result.status == 0) {
							...
							if (baziResult.isPayed) {
								...
								var year = baziResult.birth.substring(0, 4),
								...
								if (/payresult=1/.test(location.href)) {
									console.log(result);
									var shareUrl = "//mobile.51wnl.com/numberology/Tools/bzcs_index.html?userId=[WNLUSERID]&deviceId=[OPENUDID]&mac=[MAC]&imei=[IMEI]&idfa=[IDFA]&channel=[CHANNEL]&posId=[posId]&boundId=[BUNDLE]&pushToken=[PTOKEN]&pToken=[PTOKEN]";
									console.log('redp');
									window.shareRedPackage({
										...
										orderId: orderId,
										url: shareUrl,
										wxShareTitle: '我在万年历做的八字测算，好准！你也来看看？',
										wxShareText: '我在万年历做的八字测算，好准！你也来看看？',
										wxShareImage: '1',
...
function appCallback_share() {
	...
	var link = location.href.replace('&payresult=1', '') + "&share=1";
