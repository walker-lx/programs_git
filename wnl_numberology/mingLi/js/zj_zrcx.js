$(function () {
    ...
    router.route("#/",function(req,next){
        $(".zj_zrcx_step1").removeClass("hidden");
        $(".zj_zrcx_step2").addClass("hidden");
        $('.pop_outbox').addClass('hidden');
        $('.bg_mask').addClass('hidden');
    });
    router.route("#/step2",function(req,next){
        $(".zj_zrcx_step1").addClass("hidden");
        $(".zj_zrcx_step2").removeClass("hidden");
    });
    router.play(1);
    ...
    $(".next_step_btn").click(function () {
        ...
        if ($(this).hasClass("btn_step1")){
            ...
            if(chuxingBornDate===""){
                drawToast("请选择出行人出生日期");
                return false;
            }
            //出生时间
            if(chuxingBornTime==="-1"){
                drawToast("请选择出行人出生时间");
                return false;
            }
            if(bornAddress===""){
                drawToast("请选择出行人出生城市");
                return false;
            }
            if(chuxingAddress===""){
                drawToast("请选择出行人出行城市");
                return false;
            }
...
function userinfocallback(result){
    ...
    if(originalObj.chuxingName&&originalObj.chuxingName.length!==0){
        $(".chuxingNameTxt").val(originalObj.chuxingName);
    }
