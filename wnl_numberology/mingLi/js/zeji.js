...
//        kaizhangBornDate="20140712";
    ...
    //加载省级的json
    $.ajax({
        url:"../mingli/data/province.json",
        type:"get",
        dataType:"json",
        success:function(result){
            $.each(result,function(){
                $(".bornProvinceSelect").append("<option value='"+this.file+"'>"+this.name+"</option>");
                $(".goProvinceSelect").append("<option value='"+this.file+"'>"+this.name+"</option>");
            });
            if(localStorage["globalLocalData"]||localStorage["zejiLocalData"]) {
                var zejiLocalData1,globalLocalData1;
                if(localStorage["zejiLocalData"]){
                    zejiLocalData1 = JSON.parse(localStorage["zejiLocalData"]);
                }
                if(localStorage["globalLocalData"]){
                    globalLocalData1 = JSON.parse(localStorage["globalLocalData"]);
                }
                var provinceCode=(zejiLocalData1&&zejiLocalData1.provinceCode)||(globalLocalData1&&globalLocalData1.provinceCode)||null;
                var cityCode=(zejiLocalData1&&zejiLocalData1.cityCode)||(globalLocalData1&&globalLocalData1.cityCode)||null;
                var countryCode=(zejiLocalData1&&zejiLocalData1.countryCode)||(globalLocalData1&&globalLocalData1.countryCode)||null;
                if(provinceCode||cityCode||countryCode){
                    $(".bornCitySelect").empty();
                    $(".bornCountrySelect").empty();
                    $(".bornProvinceSelect").val(provinceCode);
                    $.ajax({
                        url:"../mingli/data/"+provinceCode+".json",
                        type:"get",
                        dataType:"json",
                        success:function(result){
                            ...
                            $(".bornCitySelect").val(cityCode);
                            var countryData=chuxingCitySelectData[cityCode];
                            if(countryData.s){
                                ...
                                $(".bornCountrySelect").val(countryCode);
                            }
                        }
                    });
                    $(".bornCityUISelect span").html(zejiLocalData.bornCity);
                    $(".bornCountryUISelect span").html(zejiLocalData.bornCountry);
                }
            }
        }
    });
...
//        chuxingBornDate="20140712";
    ...
    $.ajax({
        ...
        success:function(result){
            $.each(result,function(){
                $(".hehunMaleBornProvinceSelect").append("<option value='"+this.file+"'>"+this.name+"</option>");
                $(".hehunFeMaleBornProvinceSelect").append("<option value='"+this.file+"'>"+this.name+"</option>");
            });
            if(maleBornPrivinceCode||maleBornCityCode||maleBornCountryCode){
                $(".hehunMaleBornCountrySelect").empty();
                $(".hehunMaleBornCitySelect").empty();
                $(".hehunMaleBornProvinceSelect").val(maleBornPrivinceCode);
                $.ajax({
                    url:"../mingli/data/"+maleBornPrivinceCode+".json",
                    type:"get",
                    dataType:"json",
                    success:function(result){
                        ...
                        $(".hehunMaleBornCitySelect").val(maleBornCityCode);
                        var countryData=citySelectData[maleBornCityCode];
                        if(countryData.s){
                            ...
                            $(".hehunMaleBornCountrySelect").val(maleBornCountryCode);
                        }
                    }
                });
                $(".hehunMaleBornCityUISelect span").html(maleBornCity);
                $(".hehunMaleBornCountryUISelect span").html(maleBornCountry);
            }
            if(femaleBornPrivinceCode||femaleBornCityCode||femaleBornCountryCode){
                $(".hehunFeMaleBornCountrySelect").empty();
                $(".hehunFeMaleBornCitySelect").empty();
                $(".hehunFeMaleBornProvinceSelect").val(femaleBornPrivinceCode);
                $.ajax({
                    url:"../mingli/data/"+femaleBornPrivinceCode+".json",
                    type:"get",
                    dataType:"json",
                    success:function(result){
                        citySelectData=result;
                        ...
                        $(".hehunFeMaleBornCitySelect").val(femaleBornCityCode);
                        var countryData=citySelectData[femaleBornCityCode];
                        if(countryData.s){
                            ...
                            $(".hehunFeMaleBornCountrySelect").val(femaleBornCountryCode);
                        }
                    }
                });
                $(".hehunFeMaleBornCityUISelect span").html(femaleBornCity);
                $(".hehunFeMaleBornCountryUISelect span").html(femaleBornCountry);
            }
        }
    });
    //男方信息省级下拉选择改变事件
