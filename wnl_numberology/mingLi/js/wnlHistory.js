(function (global) {
	...
	userId = localStorage["wnlUserId"] || getQueryValue('userId');
	if (userId !== null && userId.toLowerCase() === '[wnluserid]') {
		userId = '';
	}
	userId = userId === '' || userId == null ? '' : userId.replace('?name=', '');

	localData.userId = userId;
	deviceId = localStorage["deviceid"] || getQueryValue('deviceId');
	if (deviceId !== null && deviceId.toLowerCase() === '[openudid]') {
		deviceId = '';
	}
	localData.deviceId = deviceId;
	var posId = localStorage["posid"] || getQueryValue('posId') || getQueryValue('posid');
	localData.posId = posId;
	var pushToken = localStorage["pushToken"] || getQueryValue('pushToken') || getQueryValue('pushtoken');
	localData.pushToken = pushToken;
	var mac = localStorage["deviceMacId"] || getQueryValue('mac');
	localData.mac = mac;
	var imei = localStorage["imei"] || getQueryValue('imei');
	localData.imei = imei;
	var pToken = localStorage["pToken"] || getQueryValue('pToken') || getQueryValue('ptoken');
	localData.pToken = pToken;
	var idfa = localStorage["idfa"] || getQueryValue('idfa');
	localData.idfa = idfa;
	var boundid = localStorage["boundid"] || getQueryValue('boundid');
