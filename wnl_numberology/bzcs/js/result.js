$(function () {
	...
	var useCountStr;
	var originLink = 'http://mobile.51wnl.com/numberology/bzcs/bzcs_index.html?userId=[WNLUSERID]&deviceId=[OPENUDID]&pushToken=[PTOKEN]&pToken=[PTOKEN]&mac=[MAC]&imei=[IMEI]&idfa=[IDFA]&channel=[CHANNEL]&posId=[posId]&boundId=[BUNDLE]';

	console.log(userId);
	console.log('优惠券'+couponId);
	...
	if (browser.isIOS()) {
		money = 98;
		/* goodsid = 'E4CF3D6698834375AD72BE533A67D5E4'; */
		goodsid = '41DBA1789A644753A408CD78DAF79B00'; //正式
		...
		goodsid = 'E5DCC5ABE757410A8BB39B53170B0762'; //正式
		$('#tenYearsMoney').html(money);
	...
	$(document).on('click', '.tenYears,.eightyYears,.payForBzcs', function () {
		if ($('.tenYears').hasClass('active')) {
			...
			if(browser.isIOS()) {
			/* goodsid = '36460FC8DA6E4283B8FA3370D58E388B'; */
				goodsid = '74AF28CE574C4DB580DACD58EEFE0284'; //正式
				money = $('#eightyYearsMoney').html();
			} 
			else {
			/* goodsid = 'F9AED0D1C119428BB1D1934832E1BC54'; */
				goodsid = '51B37564B3F94E1A951F225C93EE7B1D'; //测试
				money = $('#eightyYearsMoney').html();
			}
	...
	$(document).on('click', '.payForBzcs', function () {
		...
		location.href = 'http://order.51wnl.com/pay_web/index_t.html?money=' + money + '&source=八字测算&orderName=八字测算&parterid=NRLorder&goodsid=' + goodsid + '&parteruserid=' + uniqueId +
			'&data=' + orderId + '&posId=' + posId + '&openid=' + openid + '&couponId=' + couponId + '&imei=' + imei + '&returnUrl=' + encodeURIComponent(location.href.replace('https', 'http'));
	});
				...
				$.ajax({
					...
					success: function (result) {
						...
						if (result.status == 0) {
							...
							$.each(liuNianList, function (index) {
								if (eightYearsZX == 0) {
							...
							if (baziResult.isPayed) {
								...
								if (/payresult=1/.test(location.href)) {
									...
									if(result.data.liuNian.length >20) {
										if(browser.isIOS()) {
											...
											goodsId = '51B37564B3F94E1A951F225C93EE7B1D'; //正式
										}
									} 
									else {
										if(browser.isIOS()) {
								...
								bzcs_year = baziResult.birth.substring(0, 4);
								bzcs_year = bzcs_year + '年';
								bzcs_month = baziResult.birth.substring(5, 7);
								bzcs_month = bzcs_month + '月';
								bzcs_day = baziResult.birth.substring(8, 10);
								bzcs_day = bzcs_day + '日';
								bzcs_hour = baziResult.birth.substring(11, 13);
								...
								if (baziResult.liuNian.length > 20) {
									dayunList = baziResult.daYunYear;
								...
								$('.bzcs_bz_time').text(bzArray[3]);
								$('.wxContentTitle').removeClass('hidden');
								length = Math.floor($('.xingqingDetail').width() / 15);
								...
								//大运格式不对
								dayunList = baziResult.daYun.split(';');
								times = new Array();
								timeDescs = new Array();
...
var baseUrl = '//mobile.51wnl.com/numberology/bzcs/bzcs_index.html?userId=[WNLUSERID]&deviceId=[OPENUDID]&pushToken=[PTOKEN]&pToken=[PTOKEN]&mac=[MAC]&imei=[IMEI]&idfa=[IDFA]&channel=[CHANNEL]&posId=[posId]&boundId=[BUNDLE]';
