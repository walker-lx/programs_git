...
let shareData = {
	...
	url: `${window.location.protocol}//${window.location.host + window.location.pathname.replace('edit', 'index')}`
};
...
$(() => {
	...
	let orderid = utils.getQueryValue('orderid');
	$('#name').val(utils.getQueryValue('name'));
	$('.birthinput_text').html(utils.getQueryValue('birth').split('-').join('.'));
	$('.birthinput').on('click', () => {
		$('.tip').addClass('slide');
			...
			$.ajax({
				...
				success: (res) => {
					...
					if (JSON.parse(res).status === 0) {
						$.toast().reset('all');
						$.toast('修改成功');
						window.location.href = `./index.html?name=${ajaxData.Name}&birth=${ajaxData.GLBirthDay}&orderid=${JSON.parse(res).data.data.orderID}`;
					}
					else {
						$.toast().reset('all');
						$.toast('修改失败');
