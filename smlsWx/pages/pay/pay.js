...
Page({
	...
	onLoad: function (options) {
		...
		paydata = {
			...
			"name": options.name,
			"glBirthDay": options.glbirthday,
		}
		this.setData({
			month: options.ordernum,
	...
	*/
	...
	pay: function () {
		...
		let jsonData = {
			...
			'ParterUserId': wx.getStorageSync('openid'),
			'Idfa': '',
		...
		wx.request({
			url: 'https://order.51wnl.com/api/PayPreOrder/getsignstr',
			method: 'get',
			data: jsonData,
			success: res => {
				console.log(res, 'res')
					...
					wx.requestPayment({
						...
						'success': function (res) {
							...
							setTimeout(() => {
								...
								getCurrentPages()[0].onLoad(paydata)
								wx.hideLoading()
							}, 4000)							
						},
						fail: function (res) {
